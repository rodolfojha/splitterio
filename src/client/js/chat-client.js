...
class ChatClient {
    ...
    registerFunctions() {
        ...
        this.registerCommand('admin', 'Open admin panel.', function () {
            window.location.href = 'admin.html';
        });

        this.registerCommand('money', 'Show money per cell.', function () {
            if (global.player && global.player.cells) {
                let totalMoney = 0;
                global.player.cells.forEach((cell, index) => {
                    const cellMoney = cell.gameMoney || 0;
                    totalMoney += cellMoney;
                    self.addSystemLine(`Célula ${index + 1}: $${cellMoney}`);
                });
                self.addSystemLine(`💰 Dinero total: $${totalMoney}`);
            } else {
                self.addSystemLine('No tienes células activas.');
            }
        });

        this.registerCommand('debugmoney', 'Toggle money debug mode.', function () {
            global.debugMoney = !global.debugMoney;
            self.addSystemLine(`Debug dinero: ${global.debugMoney ? 'ON' : 'OFF'}`);
        });

        this.registerCommand('protection', 'Show protection status per cell.', function () {
            if (global.player && global.player.cells) {
                let protectedCells = 0;
                global.player.cells.forEach((cell, index) => {
                    const isProtected = cell.isProtected || false;
                    const protectionTime = cell.protectionTimeLeft || 0;
                    const cellMoney = cell.gameMoney || 0;
                    
                    if (isProtected) {
                        protectedCells++;
                        self.addSystemLine(`🛡️ Célula ${index + 1}: Protegida por ${protectionTime}s - $${cellMoney}`);
                    } else {
                        self.addSystemLine(`⚪ Célula ${index + 1}: Sin protección - $${cellMoney}`);
                    }
                });
                
                if (protectedCells > 0) {
                    self.addSystemLine(`🛡️ Tienes ${protectedCells} células protegidas`);
                } else {
                    self.addSystemLine('⚪ No tienes células protegidas');
                }
            } else {
                self.addSystemLine('No tienes células activas.');
            }
    ...
    toggleRedZone() {
        if (!global.showRedZone) {
            global.showRedZone = true;
            this.addSystemLine('Red zone display enabled.');
        } else {
            global.showRedZone = false;
            this.addSystemLine('Red zone display disabled.');
        }
    }
}
